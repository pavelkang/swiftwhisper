...
public actor CGEventTapHotKeyMonitor: GlobalHotKeyMonitoring {
    private var runner: EventTapRunner?
    private var continuation: AsyncStream<HotKeyEvent>.Continuation?

    public init() {}

    public func events(for binding: WakeBinding) async throws -> AsyncStream<HotKeyEvent> {
    ...
    fileprivate func handle(type: CGEventType, event: CGEvent) {
        ...
        case .modifierOnly(let modifier):
            guard type == .flagsChanged,
                  keyCode == Self.keyCode(for: modifier) else { return }
